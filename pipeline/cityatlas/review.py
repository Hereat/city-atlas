...
城区十几公里外、有没有城的包里一条线都没有。前两件**只能靠眼睛**：一座城的画框
...
* **画框**：不问「中心离城区多远」，问**默认取景里画得出多少条路**。前者要另一份
  ...
  最前面，逐张画出来。
* **空包**：包里一条道路都没有的城。画框落在农田、海面或山里就会这样——
  切出来不是零字节，图上却什么都没有。
...
def _thin(city: dict) -> dict:
    ...
    不裁的是 `coastline`：那一列装的是闭合环，绕向就是它的角色（顺时针是海、逆时针是岛），
    裁掉一段等于把环拆开，nonzero 填出来是另一块形状。这一层本来也小。
    """
...
def _summary(packages, empties) -> dict:
    tails: dict[str, int] = {}
...
def _empty_check(packages) -> dict:
    """画框里一条要素都没有的城。

    先前这里判的是「`extract/` 里有没有小于 1 KB 的切片」——osmium 被内存打爆时会留下
    零字节的残件，那条判据是为它写的。换成 Overture 之后没有每城一份的切片了，
    但**换掉它的真正理由不是文件没了**：切片非空不等于图上有东西。画框落在农田、
...
/* ---------- 海面：包里就是闭合环，绕向即角色（顺时针是海、逆时针是岛） ---------- */
function coastGeometry(rings){
  const sea=new Path2D();
  for(const flat of rings){ const p=decode(flat); if(p.length<3) continue;
    sea.moveTo(p[0][0],p[0][1]); for(let i=1;i<p.length;i++) sea.lineTo(p[i][0],p[i][1]);
    sea.closePath(); }
  return sea;
}
...
function draw(canvas, city){
  ...
  if(city.coastline.length){ ctx.fillStyle=S.water; ctx.fill(coastGeometry(city.coastline),"nonzero"); }
...
<header>
  ...
  <p>三件要核的事在下面：名单里有没有非城市、有没有城的画框落在离城区十几公里外、
     有没有一条线都没有的空包。图按**默认取景**画（用户打开城市图看到的第一眼），
...
function summaryHTML(){
  const s=SUMMARY, bad=s.empties.bad;
  ...
  return `<table>
    ...
    <tr><th>空包</th><td>${s.empties.total} 座，${bad.length?`<span class="thin">一条线都没有的 ${bad.length} 座：${bad.map(b=>b[0]).join("、")}</span>`:"每座城都有线"}</td></tr>
